...
---
--- - Configurable mappings to navigate to first/previous/next/last hunk.
---   See |MiniDiff.goto_hunk()|.
--- What it doesn't do:
---
--- - Provide functionality to work directly with Git outside of visualizing
---   hunks with (default) Git source. In particular,
...
--- # Setup ~
---
--- This module is self-contained and initializes on load.
--- Edit the config table in this file to change behavior.
---
--- # Comparisons ~
---
...
--- # Mappings ~
---
--- This module provides mappings for hunk navigation:
...
--- # Mappings ~
---
--- `config.mappings` contains keys which are mapped during initialization.
---
...
H.apply_config = function(config)
  ...
  H.map({ 'n', 'x' }, mappings.goto_first, goto('first'), { desc = 'First hunk' })
  H.map({ 'n', 'x' }, mappings.goto_prev, goto('prev'), { desc = 'Previous hunk' })
  H.map({ 'n', 'x' }, mappings.goto_next, goto('next'), { desc = 'Next hunk' })
  H.map({ 'n', 'x' }, mappings.goto_last, goto('last'), { desc = 'Last hunk' })
