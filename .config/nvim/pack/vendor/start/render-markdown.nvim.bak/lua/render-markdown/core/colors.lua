...
local function notify_ready()
    vim.api.nvim_exec_autocmds('User', { pattern = 'RenderMarkdownColors' })
end

---@private
local function ensure_code_background()
    local code = M.get_hl(M.prefix .. 'Code')
    if code.bg ~= nil or code.ctermbg ~= nil then
        return
    end
    for _, group in ipairs({ 'CursorLine', 'CursorColumn', 'ColorColumn' }) do
        local hl = M.get_hl(group)
        local bg = hl.bg or hl.ctermbg
        if bg ~= nil then
            vim.api.nvim_set_hl(0, M.prefix .. 'Code', {
                bg = hl.bg,
                ctermbg = hl.ctermbg,
                fg = code.fg,
                ctermfg = code.ctermfg,
            })
            return
        end
    end
...
function M.init()
    ...
    vim.api.nvim_create_autocmd('VimEnter', {
        group = vim.api.nvim_create_augroup('RenderMarkdownColorsOnce', {}),
        callback = function()
            ensure_code_background()
            notify_ready()
        end,
