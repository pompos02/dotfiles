...
function Render:sign_column(language)
    local text, highlight = self:sign_text(language)
    if not text then
        return
    end
    self:sign(self.config, self.config.sign, text, highlight)
end

---@private
---@param language? render.md.Node
---@return string?, string?
function Render:sign_text(language)
    if not self.config.sign then
        return nil, nil
    end
    local text = self.config.sign_text
    if type(text) == 'function' then
        text = text({ language = language and language.text or nil })
    end
    if text and text ~= '' then
        return text, self.config.highlight_language or self.config.highlight_fallback
    end
    return self:language_icon(language)
end

---@private
---@param language? render.md.Node
---@return string?, string?
function Render:language_icon(language)
...
function Render:language(info, language, delim)
    ...
    local icon, icon_hl = self:language_icon(language)
