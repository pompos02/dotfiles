...
const float DURATION = 0.04; //IN SECOND
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec2 shortenedPrev = mix(currentCursor.xy, previousCursor.xy, 0.5); // 50% toward previous
    vec2 v0 = vec2(currentCursor.x + currentCursor.z * vertexFactor, currentCursor.y - currentCursor.w);
    vec2 v1 = vec2(currentCursor.x + currentCursor.z * invertedVertexFactor, currentCursor.y);
    vec2 v2 = vec2(shortenedPrev.x + currentCursor.z * invertedVertexFactor, shortenedPrev.y);
    vec2 v3 = vec2(shortenedPrev.x + currentCursor.z * vertexFactor, shortenedPrev.y - mix(currentCursor.w, previousCursor.w, 0.5));
