...
theme_apply_alacritty() {
	local config_file tmp line section in_managed_block=0 in_color_section=0
	local -a config_lines
	config_file="$(theme_find_alacritty_config)"
	[[ -n "$config_file" ]] || return 0
	[[ "$config_file" == *.toml ]] || return 0

	tmp="$(mktemp "${config_file}.tmp.XXXXXX")" || return 1
	while IFS= read -r line || [[ -n "$line" ]]; do
		if [[ "$line" == '# BEGIN_ALACRITTY_THEME' ]]; then
			in_managed_block=1
			continue
		fi

		if [[ "$in_managed_block" == 1 ]]; then
			[[ "$line" == '# END_ALACRITTY_THEME' ]] && in_managed_block=0
			continue
		fi
		...
		if [[ "$line" =~ ^[[:space:]]*\[([^]]+)\][[:space:]]*$ ]]; then
			section="${BASH_REMATCH[1]}"
			case "$section" in
			colors.primary | colors.cursor | colors.selection | colors.normal | colors.bright)
				in_color_section=1
				continue
				;;
			*)
		...
		[[ "$in_color_section" == 1 ]] && continue
		printf '%s\n' "$line" >>"$tmp"
		done <"$config_file"

	mapfile -t config_lines <"$tmp"
	while ((${#config_lines[@]} > 0)) && [[ -z "$(trim "${config_lines[-1]}")" ]]; do
		unset 'config_lines[-1]'
	done
	: >"$tmp"
	...
	theme_write_alacritty_colors >>"$tmp"
	mv "$tmp" "$config_file"
}
