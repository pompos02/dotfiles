...
THEME_SYSTEM_FILE="${HOME}/.config/system-theme"
...
theme_write_system_variant() {
	: "${THEME_VARIANT:?theme_write_system_variant requires a loaded theme}"
	printf '%s\n' "$THEME_VARIANT" >"$THEME_SYSTEM_FILE"
}
...
theme_ensure_kitty_current_file() {
	if [[ -f "$THEME_KITTY_CURRENT_FILE" ]]; then
		return 0
	fi

	cat >"$THEME_KITTY_CURRENT_FILE" <<'EOF'
# vim:ft=kitty
...
inactive_border_color   #52494e

# Tab bar colors
active_tab_foreground   #e4e4ef
...
_theme_kitty_replace() {
	local key="$1"
	local value="$2"
	local replacement

	replacement="$(_theme_escape_sed_replacement "$value")"
	sed -i -E "s|^(${key}[[:space:]]+).*$|\\1${replacement}|" "$THEME_KITTY_CURRENT_FILE"
}

theme_apply_kitty() {
	theme_ensure_kitty_current_file

	sed -i -E "s|^## name:.*$|## name: $(_theme_escape_sed_replacement "$THEME_NAME")|" "$THEME_KITTY_CURRENT_FILE"
	_theme_kitty_replace foreground "$THEME_FOREGROUND"
	_theme_kitty_replace background "$THEME_BACKGROUND"
	_theme_kitty_replace selection_foreground "$THEME_FOREGROUND"
	_theme_kitty_replace selection_background "$THEME_SELECTION"
	_theme_kitty_replace cursor "$THEME_FOREGROUND"
	_theme_kitty_replace cursor_text_color "$THEME_BACKGROUND"
	_theme_kitty_replace active_border_color "$THEME_BLUE"
	_theme_kitty_replace inactive_border_color "$THEME_BORDER"
	_theme_kitty_replace active_tab_foreground "$THEME_FOREGROUND"
	_theme_kitty_replace active_tab_background "$THEME_SURFACE"
	_theme_kitty_replace inactive_tab_foreground "$THEME_MUTED"
	_theme_kitty_replace inactive_tab_background "$THEME_BACKGROUND_ALT"
	_theme_kitty_replace color0 "$THEME_BACKGROUND_ALT"
	_theme_kitty_replace color1 "$THEME_RED"
	_theme_kitty_replace color2 "$THEME_GREEN"
	_theme_kitty_replace color3 "$THEME_YELLOW"
	_theme_kitty_replace color4 "$THEME_BLUE"
	_theme_kitty_replace color5 "$THEME_MAGENTA"
	_theme_kitty_replace color6 "$THEME_CYAN"
	_theme_kitty_replace color7 "$THEME_SURFACE_ALT"
	_theme_kitty_replace color8 "$THEME_MUTED"
	_theme_kitty_replace color9 "$THEME_RED"
	_theme_kitty_replace color10 "$THEME_GREEN"
	_theme_kitty_replace color11 "$THEME_ORANGE"
	_theme_kitty_replace color12 "$THEME_LAVENDER"
	_theme_kitty_replace color13 "$THEME_LAVENDER"
	_theme_kitty_replace color14 "$THEME_CYAN"
	_theme_kitty_replace color15 "$THEME_FOREGROUND"
...
theme_fzf_args() {
	theme_source_env >/dev/null 2>&1 || true
	_theme_default_palette
	THEME_FZF_ARGS=(
		--border=rounded
		--info=right
...
theme_pick() {
	...
	theme_fzf_args
	selected="$(theme_picker_entries | fzf --delimiter=$'\t' --with-nth=1 --header "$header" "${THEME_FZF_ARGS[@]}")" || return 1
	[[ -n "$selected" ]] || return 1
...
Commands:
  ...
  env             Regenerate current_theme.env for the current theme
EOF
}
